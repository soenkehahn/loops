...
toSignal = \case
  TakeConstant value length -> Signal (Finite length) $ do
    return $ \time -> do
      return $
        if not (time `lt` 0) && time `lt` length
          then value
          else error "boo"
  Ramp from to length ->
    ramp from to length
...
goldenTests = describe "property tests" $ do
  it "behaves like an unoptimized simpler version of divide" $
    property $
      forAllShrink (choose (0, 100)) shrink $ \(Time -> length) ->
        (0 `lt` length)
          ==> forAllShrink (choose (0, fromTime length)) shrink
          $ \(Time -> time) ->
            (0 `lt` time && time `lt` length)
              ==> forAllShrink (listOf1 (choose (1, 10))) shrink
              $ \(intWeights :: [Int]) ->
                all (> 0) intWeights
                       ...
                       in \testSignals ->
                            let parts = map (\(weight, testSignal) -> weight ~> \_time -> toSignal testSignal) $ Prelude.zip weights testSignals
                             in counterexample (show $ Prelude.zip weights testSignals) $ do
                                  end (divide parts length)
                                    `shouldBeCloseTo` end (simpleDivide parts length)
                                  getSample (divide parts length) time
                                    `shouldBeCloseTo` getSample (simpleDivide parts length) time
