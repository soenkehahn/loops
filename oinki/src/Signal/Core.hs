...
data Signal a = Signal_ {
  end :: Length,
  initialize :: forall s . ST s (Time -> ST s a)
} deriving (Functor)

instance Applicative Signal where
  pure a = Signal_ Infinite $ return $ \ _time -> return a
  fSignal <*> xSignal =
    Signal_ (minLength (end fSignal) (end xSignal)) $ do
...
simpleSignal function = Signal_ Infinite $ do
  return $ \ time -> do
