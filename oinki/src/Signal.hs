...
take (Signal_ mEnd signal) length = Signal_ end' signal
  where
...
skip length signal =
  Signal_ (mapLength (\ end -> maxTime 0 (end - length)) (end signal)) $ do
...
fromList delta (Vec.fromList -> vec) =
  Signal_ (Finite $ fromIntegral (length vec) * delta) $ do
    return $ \ time -> do
...
empty :: Signal a
empty = Signal_ (Finite 0) $ return $ \ time ->
  error ("empty: shouldn't be called. (" ++ show time ++ ")")

zip :: Signal a -> Signal b -> Signal (a, b)
zip a b = Signal_ (minLength (end a) (end b)) $ do
  runA <- initialize a
  runB <- initialize b
  return $ \ time -> do
    a <- runA time
...
zipWith f a b = Signal_ (minLength (end a) (end b)) $ do
  runA <- initialize a
  runB <- initialize b
  return $ \ time -> do
    a <- runA time
...
phase :: Signal Double
phase = Signal_ Infinite $ return $ \ time ->
...
constSpeedup (Time -> factor) signal =
  Signal_ (mapLength (/ factor) (end signal)) $ do
    runSignal <- initialize signal
    return $ \ time -> do
...
speedup factorSignal inputSignal = case end inputSignal of
  Infinite -> Signal_ (end factorSignal) $ do
...
integral signal = Signal_ (end signal) $ do
  ref <- newSTRef 0
...
a |> b = case end a of
  Infinite -> a
  Finite aEnd ->
    Signal_ (mapLength (+ aEnd) (end b)) $ do
      runA <- initialize a
      runB <- initialize b
      return $ \ time -> do
        if time `lt` aEnd
...
cycle signal = case end signal of
  Infinite -> signal
  Finite end -> Signal_ Infinite $ do
    runSignal <- initialize signal
    return $ \ time ->
...
a +++ b =
  ...
  in Signal_ (maxLength (end a) (end b)) $ do
    runA <- initialize a
    runB <- initialize b
    return $ \ time -> do
...
ramp start end length = Signal_ (Finite length) $ return $ \ time ->
